...
const Hero = () => {
  ...
  // const handleSearch = (selectedItemId) => {
  //  navigate(`/findcommunity/${selectedItemId}`)
  // };
  ...
  return (
    <div style={{
      ...
      height: '500px',
        ...
        {/* <select value={selectedSubMarket} onChange={handleSelect} className='sm:w-[500px] md:w-[530px] h-9 rounded-lg mt-3 text-green-600' >
            <option value="">Select SubMarket:</option>
            {subMarket.map(item => (
              <option key={item._id} value={item.name}>{item.name}</option>
            ) )}
          </select> */}
            <select value={selectedSubMarket} onChange={(e) => handleItemSearch(e.target.value)} className='sm:w-[500px] md:w-[530px] h-9 rounded-lg mt-3 text-green-600' >
            <option value="">Select SubMarket:</option>
            {subMarket.map(item => (
              <option key={item._id} value={item.name}>{item.name}</option>
            ) )}
